...
Contain the subject code for each observation.

* `test/y_test.txt` and `train/y_train.txt`

Contain the activity code that the subject was doing in that 
...
1. All activities are read from `activity_labels.txt` as a activity
   table, with the activity id (variable `activity_id`) as the key;
...
4. The subjects from each observation are read from 
   ...
   as a new variable called `subject`;
5. The activities are read from `train/y_train.txt` and are added to
   the train table as new variable called `activity_id`;
6. The activities labels, corresponding to each `activity_id`, are
   merged with the train table using the `activity_id` variable, that
   exists in the activity table and in the train table, as the
   merging variable;
7. The steps from 3 to 6 are repeated for the *test* set;
8. Both tables, the *train* and the *test* are merged in one table;
9. The observation table is filtered to keep only the variables listed
   in the `variables description` below, that contains only the mean
   and standard deviation values, except for the `angle` values;
